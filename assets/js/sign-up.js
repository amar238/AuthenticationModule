...
function onClick(e) {
  ...
  grecaptcha.ready(function () {
    grecaptcha
      .execute("6LcoRYEpAAAAAGsame5vBlmyHlrcNJSJRNbc6n45", { action: "submit" })
      .then(function (token) {
        ...
        if (!regexName.test(firstName)) {
          showErrorAlert(
            "First Name",
            "Input field should contain only alphabates!"
          );
          throw new Error();
        }
...
function showSuccessAlert(title, message) {
  Swal.fire({
    ...
    footer:
      '<a href="/sign-in" class="bg-green-500 text-white py-2 px-4 rounded">Sign In</a>',
  });
}
